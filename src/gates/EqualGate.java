package gates;

import java.awt.Color;

import circuits.CircuitPanel;
import circuits.Port;
import gui.Label;
import util.Point;
import util.Rectangle;
import util.Util;

public class EqualGate extends Gate {
	private static final Color EqualColor = new Color(0.45f, 0.45f, 0.85f, 1.0f);

	public static Color getEqualColor() {
		return EqualColor;
	}

	public EqualGate(CircuitPanel panel, Point position, int size) {
		super(panel, new Rectangle(position, new Point(180, 200)), EqualColor, size, "EQUAL");

		Label lbl = new Label(this, new Rectangle(0, 0, getWidth(), getHeight()), "");
		lbl.setBackground(EqualColor);
		lbl.setFontSize(40);

		if (size == 1) {
			lbl.setText("EQU");
		} else {
			lbl.setText(size + " EQU");
		}

		this.addComponent(lbl);

		this.addPort(new Port(this, new Point(90, 30), Port.OUTPUT, 1));
		this.addPort(new Port(this, new Point(45, 170), Port.SINGLE_INPUT, getBitSize()));
		this.addPort(new Port(this, new Point(135, 170), Port.SINGLE_INPUT, getBitSize()));
	}

	@Override
	public boolean[] getOutputState(int index) {
		boolean[] A = getInputStates(1);
		boolean[] B = getInputStates(2);

		return new boolean[] { Util.XNOR1(A, B) };
	}

	public static Gate loadGate(CircuitPanel panel, Point location, String other, int size) {
		return new EqualGate(panel, location, size);
	}
}
