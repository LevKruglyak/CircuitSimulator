...
public class ANDGate extends Gate {
	...
	private static final Color ANDColor = new Color(1f,0.9f,0.8f,1.0f); 

	public static Color getANDColor() {
		return ANDColor;
	}
	
	public ANDGate(CircuitPanel panel, Point position, int size) {
		super(panel, new Rectangle(position,new Point(180,200)), ANDColor, size, "AND");
		
		Label lbl = new Label(this, new Rectangle(0,0,getWidth(),getHeight()),"");
		lbl.setBackground(ANDColor);
		lbl.setFontSize(40);
		
		if (size == 1) {
			lbl.setText("AND");
		} else {
			lbl.setText(size + " AND");
		}
	...
	public static Gate loadGate(CircuitPanel panel, Point location, String other, int size) {
		return new ANDGate(panel,location, size);
	}
